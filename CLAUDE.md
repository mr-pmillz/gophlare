...
# Regenerate Flare API models (oapi-codegen, pinned as a go.mod tool)
make generate
# Re-download Flare's published Firework v2/v4 OpenAPI specs
make openapi-specs
```
...
- `phlare/flareClient.go` — `FlareClient` handles API auth (raw API key in the `Authorization` header → bearer token), refreshes the token in place after 45 minutes (Flare tokens last 1 hour; `refresh_token_exp` is the refresh token's expiry, not the API token's), and makes all Flare API calls (global events search, credentials search, cookies search, stealer log downloads, bulk accounts). Pagination uses a `Next` cursor field.
- `phlare/types.go` — `FlareClient` plus type aliases (`FlareSearchCredentialsASTP`, `FlareEventsGlobalSearchResults`, …) to the generated models in `flareapi/`. Do not hand-write API structs here; change the spec and regenerate.
- `phlare/time.go` — `FlareTime`, an alias of `flaretime.Time`.
...
### Generated API Models (`flareapi/`)

- `flareapi/openapi/` — Specs. `firework-v2-openapi.json` and `firework-v4-openapi.json` are Flare's published specs, stored verbatim (refresh with `make openapi-specs`); `firework-v2-overlay.yaml` patches an upstream v2 bug at generation time. `astp-openapi.yaml`, `tokens-openapi.yaml`, and `firework-v2-events-openapi.yaml` are hand-authored from Flare's prose docs, because those endpoints (and the v2 activity payload) have no published schema.
- `flareapi/{fireworkv2,fireworkv4,astp,tokens}/` — `*.gen.go` generated by oapi-codegen from `oapi-codegen*.yaml` via `go:generate` in `generate.go`. Never edit `*.gen.go`; run `make generate`.
- `flareapi/flaretime/` — `flaretime.Time`, mapped to every OpenAPI `date-time` so generated models decode the API's mixed timestamp formats.
- Optional/nullable fields are values with `omitzero` (not pointers): JSON null decodes to the zero value. Query unions are set with generated `From*Query` methods.
...
- **FlareTime**: Custom time type (`flaretime.Time`) that handles multiple Flare API timestamp formats. Used throughout the generated API models and GORM models. When adding new time fields, use `FlareTime` not `time.Time`; in specs, `format: date-time` maps to it automatically.
- **API drift**: When Flare's API changes, update the spec in `flareapi/openapi/` (download or edit), run `make generate`, then fix compile errors in `phlare/`.
- **API pagination**: Flare API uses cursor-based pagination via a `Next string` field (JSON null decodes to `""`). Pagination loops use labeled `break flarePaginate` and `isLastPage()`.
